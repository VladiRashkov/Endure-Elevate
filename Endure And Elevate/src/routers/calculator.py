...
def update_paces():
    ...
    pace_data[changed_km] = convert_seconds_to_minutes(new_pace_seconds).strftime("%H:%M:%S")

    pace_data = distribute_the_change(pace_data, changed_km, total_seconds)

    session["pace_data"] = pace_data

    pace_data_seconds = {km: take_the_seconds(pace) for km, pace in pace_data.items()}
